...
\begin{equation}
  \mathbb{E}_{X_j, Y_j|z_j} \left[\mathbb{E}_{R_i, R_+ | X_j, Y_j, z_i}\left[\frac{R_i(X_j, Y_j)} {R_+(X_j, Y_j)}\right]\right] = \mathbb{E}_{X_j, Y_j|z_j} \left[\frac{\mathbb{E}_{R_i | X_j, Y_j, z_i}[R_i(X_j, Y_j)]} {\mathbb{E}_{R_+ | X_j, Y_j, z_i}[R_+(X_j, Y_j)]}\right].
  (\#eq:expratio)
\end{equation}
This would be true if $R_i$ and $R_+$ were independent given the place and time $(X_j, Y_j)$ of offspring $j$'s birth, and female $i$'s covariates $z_i$.  But firstly $R_+$ is a sum that includes $R_i$, and secondly they are likely to be influenced by common latent variables such as food abundance.  It seems that we can take such latent variables into account if we ignore the effect of $R_i$ on $R_+$, which might at least give a good approximation, given that the effect of any particular individual should be quite small.  Assuming that the reproductive output $R_k$ of each mature female $k = 1, ..., N_F$, at the place and time $(X_j, Y_j)$ of offspring $j$'s birth is i.i.d. given any necessary latent variables $\theta$ and female $i$'s covariates $z_i$,
