...
$$\approx E\left\{\sum_{f = 1}^{M_{b - 1}}E(SFP_{t, b, f})\right\},$$
the approximation obtained by treating $SFB_{t, b, f}$ for different fathers $f$ as independent, although they are not because if one father has more calves there are fewer mature females and thus calves available for the others.  Then
$$E(SFP_{t, b}) \approx E\left\{M_{b - 1}E(SFP_{t, b, f})\right\}$$
...
$$= {n \choose 2} p^2.$$
Then
$$E(SFP_{t, b}) \approx E(M_{b - 1})E\left[E\left\{{SFB_{t, b, f} \choose 2}\Big|B_b, M_{b - 1}\right\}\right]$$
...
the approximation obtained by treating $B_b$ and $M_{b - 1}$ as independent although they are both dependent on $N_{b - 1}$,
...
the latter approximation being more accurate as $M_{b - 1} \rightarrow \infty$ and the derivative of $\frac{1}{M_{b - 1}} \rightarrow 0$ so that it is close to a linear (constant) function of $M_{b - 1}$.  Then
...
$$= E\left[E\left\{{F_b \choose 2}\Big|N_{t - \alpha}\right\}\right]\frac{\beta^2 \phi^{2(t - b)}}{E(M_{b - 1})}$$
$$= E\left\{{N_{t - \alpha} \choose 2}\left(\frac{\phi^\alpha}{2}\right)^2 \right\}\frac{\beta^2 \phi^{2(t - b)}}{E(M_{b - 1})}$$
$$= E\left\{{N_{t - \alpha} \choose 2} \right\}\frac{\beta^2 \phi^{2 (t - b + \alpha)}}{2 E(M_{b - 1})}$$


**End of current work**

$$= E(E(SMP_{t, b_1, b_2}|B_{b_1}))$$
$$= E(B_{b_1}\phi^{2(t - b_1)}\beta)$$
$$= E(B_{b_1})\phi^{2(t - b_1)}\beta$$
$$= E(N_{b_1})\left(1 - \frac{\phi}{\lambda}\right)\phi^{2(t - b_1)}\beta$$
$$= \frac{E(N_t)}{\lambda^{t - b_1}}\left(1 - \frac{\phi}{\lambda}\right)\phi^{2(t - b_1)}\beta$$
