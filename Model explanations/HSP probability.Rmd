...
The probability that the mother has another calf at time $b_2$, given that she survives until then, is then $E\left(\frac{B_{b_2}}{F_{b_2}}\right)$.  The terms in this fraction are not necessarily independent because it is often the case that the more mature females there are, the more animals are likely to be born.  In particular we expect that to be true of NZSRW's for the duration of our current study.  The extent to which that dependence effects the value of the expectation can be investigated by simulation.
