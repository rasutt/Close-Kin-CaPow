...
Here the $R$s are functions of the random variables $X$ and $Y$, but can also themselves be random.  The inner expectations are over any randomness in those functions, while the outer expectation is over their random inputs.  I'm not really sure why they have to be separated though, maybe just to emphasize the different sources of uncertainty, conditioning on covariates, and the assumptions of independence mentioned below.

The expression seems to assume that $R_+$ is independent of $R_i$ and $z_i$, so that it need not be conditioned on the latter, and the ratio of expectations with the former is equal to the expectation of the ratio with it.  These independencies do not seem to be true, but for populations of any reasonable size the impact of $R_i$ and $z_i$ on $R_+$ will be insignificant.
