...
- $A_t$ be the event that a given animal in the population is alive at time $t$, and
- $\phi := P(A_{t + 1}|A_t)$ be the (constant) individual survival rate of animals in the population.

Assuming that survival satisfies the Markov property $P(A_{t + 1}|A_t) = P(A_{t + 1}|A_t, A_{t - 1}, ...)$, i.e. that survival at later times is independent of survival at previous times, then
...
This actually also relies on the fact that $N_t \ge 0$ and thus $E(N_t) \ge 0$.

Let 

- $B_t$ be the number of animals that are born at time $t$.  Then
$$B_t = N_t - S_{t - 1},$$
and
$$E(B_t) = E(N_t - S_{t - 1})$$
$$= E(N_t) - E(S_{t - 1})$$
...
\begin{equation}
  ...
  (\#eq:expBt)
\end{equation}
where $\lambda - \phi \ge 0$ from \@ref(eq:philambda).
